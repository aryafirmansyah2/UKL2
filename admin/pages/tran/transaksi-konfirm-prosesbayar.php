...
if ($_POST) {
    ...
    if (empty($jumlah_bayar)) {
        echo "<script>alert('Jumlah tidak boleh kosong');location.href='transaksi-konfirm-pilih.php?id=" . $_GET['id'] . "';</script>";
    } else if ($jumlah_bayar < $tagihan) {
        echo "<script>alert('Maaf uang anda kurang');location.href='transaksi-konfirm-pilih.php?id=" . $_GET['id'] . "';</script>";
    } else {
        include "koneksi.php";
